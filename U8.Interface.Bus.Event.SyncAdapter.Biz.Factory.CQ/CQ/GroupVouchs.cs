...
namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    public class GroupVouchs : U8.Interface.Bus.Event.SyncAdapter.Biz.BizBase
    {

        private string _ccode; 

        public GroupVouchs(ref ADODB.Connection conn,string ccode, string ufConnStr, string _opertype)
            : base(conn, ufConnStr)
        {

            oracleTableName = "MES_CQ_GroupVouchs";   //目标表名
            oraclePriKey = "dhcode";      //目标表主键 
            fieldcmpTablename = "MES_CQ_GroupVouchs";
            ufTableName = "GroupVouchs";  // "SaleOrderSQ";      //来源表名
        ...
        public override DataTable GetSourceData(string sourceTableName, string sourceKeyName, string sourceKeyValue, string colNames)
        {
            ...
            _tempsourcetable += " from GroupVouchs b with(nolock) inner join PU_ArrivalVouch t on t.id = b.id ";
