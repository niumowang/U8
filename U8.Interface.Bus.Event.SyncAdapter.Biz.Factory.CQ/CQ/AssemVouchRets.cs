namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    public class AssemVouchRets : BizBase
    {
        private string _cAVCode;

        public AssemVouchRets(ref Connection conn, string cAVCode, string ufConnStr, string _opertype) : base(conn, ufConnStr)
        {
            base.oracleTableName = "MES_CQ_AssemVouchRets";
            base.oraclePriKey = "cAVCode";
            base.fieldcmpTablename = "MES_CQ_AssemVouchRets";
        ...
        public override StringBuilder CreateDeleteString()
        {
            ...
            {
                return new StringBuilder(" DELETE FROM " + base.oracleTableName + " WHERE  " + base.oracleTableName + ".cAVDID in( select  autoid from " + base.ufTableName + " sb with(nolock) INNER JOIN AssemVouch sh WITH(NOLOCK) ON sh.id = sb.id where sh.cVouchType = '14' AND sh.cAVCode ='" + this._cAVCode + "') ");
            }
        ...
        public override DataTable GetSourceData(string sourceTableName, string sourceKeyName, string sourceKeyValue, string colNames)
        {
            ...
            string str2 = "SELECT " + colNames + " FROM " + base.ufTableName + " WHERE ID IN ( SELECT ID FROM AssemVouch WITH(NOLOCK) WHERE cVouchType = '14'  AND cAVCode ='" + this._cAVCode + "'  ) ";
