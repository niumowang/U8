namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    public class Mom_orderdetail : BizBase
    {
        ...
        public Mom_orderdetail(ref Connection conn, int moid, int modid, string ufConnStr, string _opertype) : base(conn, ufConnStr)
        {
            base.oracleTableName = "MES_CQ_mom_order";
            base.oraclePriKey = "MoCode";
            base.fieldcmpTablename = "MES_CQ_Mom_orderdetail";
            base.ufTableName = "v_mom_orderdetail_wf";
            base.ufPriKey = "MoCode";
        ...
        {
            string str = " ( SELECT d.*,h.MoCode FROM v_mom_orderdetail_wf d with(nolock) inner join v_mom_order_wf h with(nolock) on h.moid = d.moid ) tmp ";
