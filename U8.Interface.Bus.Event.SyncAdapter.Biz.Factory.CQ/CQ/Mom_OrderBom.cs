...
namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ 
{
    ...
    { 
        ...
        public Mom_OrderBom(ref ADODB.Connection conn, int moid, int modid, string ufConnStr, string _opertype)
            : base(conn, ufConnStr)
        {

            oracleTableName = "MES_CQ_Mom_orderdetail";   //目标表名
            oraclePriKey = "MoCode";      //目标表主键 
            fieldcmpTablename = "MES_CQ_Mom_OrderBom";
        ...
        {
            string _tempTable = " ( SELECT d.*,h.MoCode,dd.BomId FROM v_mom_orderdetail_wf d with(nolock) inner join v_mom_order_wf h with(nolock) on h.moid = d.moid inner join  mom_orderdetail dd with(nolock) on dd.modid = d.modid) tmp ";
