namespace U8.Interface.Bus.ApiService.Voucher.OP.Factory.CQ
{
    ...
    public class RdRecord09 : StockOP
    {
        ...
        public override string GetCodeorID(string strID, BaseData bd, string codeorid)
        {
            ...
            if (codeorid == "id")
            {
                str = "select isnull(id,'') from RdRecord09  with(nolock)  where ccode='" + strID + "'";
            }
            if (codeorid == "code")
            {
                str = "select isnull(ccode,'') from RdRecord09  with(nolock)  where id='" + strID + "'";
            }
            ...
            Common.ErrorMsg(str2, "未能获其它出库单ID或单号");
            return str2;
        }
        ...
        {
            Synergismlogdt synergismlogdt = new Synergismlogdt();
            synergismlogdt.set_Autoid(autoid.ToString());
            synergismlogdt.set_Id(autoid.ToString());
