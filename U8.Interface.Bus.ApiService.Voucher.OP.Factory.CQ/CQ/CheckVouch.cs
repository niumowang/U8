namespace U8.Interface.Bus.ApiService.Voucher.OP.Factory.CQ
{
    ...
    public class CheckVouch : StockOP
    {
        ...
        public override string GetCodeorID(string strID, BaseData bd, string codeorid)
        {
            ...
            if (codeorid == "id")
            {
                str = "select isnull(id,'') from CheckVouch  with(nolock)  where cCVCode ='" + strID + "'";
            }
            if (codeorid == "code")
            {
                str = "select isnull(cCVCode,'') from CheckVouch  with(nolock)  where id ='" + strID + "'";
            }
        ...
        public override DataSet SetFromTabet(Synergismlogdt dt, Synergismlogdt pdt, APIData apidata)
        {
            ...
            string str2 = (((("select " + " '" + connectInfo.get_UserId() + "' as cMaker,") + "'" + DateTime.Now.ToString(SysInfo.datetimeFormat) + "' as dnmaketime, ") + " lt.cCVCode as mes_cCVCode ," + " lt.dCVDate as mes_dCVDate ,") + " lt.dACDate as mes_dACDate ," + " lt.cDepCode as mes_cDepCode ,") + " lt.cPersonCode as mes_cPersonCode ," + " lt.cWhCode as mes_cWhCode ";
        ...
        {
            ConnectInfo connectInfo = ClassFactory.GetITaskLogDetailDAL(apidata.get_TaskType()).GetConnectInfo(pdt);
            string str2 = ((((("select  " + " lt.cCVCode as mes_cCVCode ," + " lt.dCVDate as mes_dCVDate ,") + " lt.dACDate as mes_dACDate ," + " lt.cDepCode as mes_cDepCode ,") + " lt.cPersonCode as mes_cPersonCode ," + " lt.cWhCode as mes_cWhCode, ") + " lt.cIRdCode as MES_cIRdCode, " + " lt.cORdCode as MES_cORdCode, ") + " CASE lb.opertype WHEN 0 THEN 'A' WHEN 1 THEN 'M' WHEN '2' THEN 'D' ELSE 'A' END as editprop, " + " lb.cinvcode as mes_cinvcode ,lb.cVenCode as MES_B_cVenCode,  ") + " lb.iquantity as MES_iquantity,   " + " lb.cVenCode as mes_cvencode   ";
            string str = str2 + " from   " + this.bodytable + " lb with(nolock) INNER JOIN " + this.headtable + " lt with(nolock) on lt.id = lb.id where lt.id ='" + pdt.get_Id() + "' ";
            DataSet set = new DbHelperSQLP(connectInfo.get_Constring()).Query(str);
        ...
        public override int Update(Synergismlog dt)
        {
            ...
            {
                builder.Append(" cCVCode = '" + dt.get_Cvoucherno() + "',  ");
            }
